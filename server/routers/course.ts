...
export const courseRouter = router({
  ...
  course: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!course) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Course not found" });
      }

      const price = await stripe.prices.list({ product: course.stripeProductId, limit: 1 });

      if (!price) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Price not found for course" });
      }
